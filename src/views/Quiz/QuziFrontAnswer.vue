...
export default{
    ...
    //計算總頁數
        totalPages() {
            return Math.ceil(this.fillobject.questions.length / this.itemsPerPage);
        },
    },
    methods:{
        changePage(newPage) {
    ...
    mounted(){
        this.quizIndex = JSON.parse(localStorage.getItem("quizIndex"))
            ...
            <div class="writebox">
                ...
                <div class="font quizList">
                    <div class="kk" style="display: flex;">
                        <p class="font">姓名：</p>
                        <input class="inputinfo" type="text" name="" id="" v-model="this.writername" style="margin-left: 30px;">
                    </div>
                    <div class="kk" style="display: flex;">
                        <p class="font">手機：</p>
                        <input class="inputinfo" type="text" name="" id="" v-model="this.writerphone" style="margin-left: 30px;" onkeyup="value=value.replace(/[^\d]/g,'') ">
                    </div>
                    <div class="kk" style="display: flex;">
                        <p class="font">E-mail：</p>
                        <input class="inputinfo" type="text" name="" id="" v-model="this.writermail" style="margin-left: 9px;">
                    </div>
                    <div class="kk" style="display: flex;">
                        <p class="font">年齡：</p>
                        <input class="inputinfo" type="text" name="" id="" v-model="this.writerage" style="margin-left: 30px;" onkeyup="value=value.replace(/[^\d]/g,'') ">
                    </div>
                    <div class="columBox" v-if="dataLoaded && fillobject != null">
                        <!-- 第一個問題是questionIndex，第一個答案是optionIndex -->
                        <div class="colum" v-for="(question, questionIndex) in this.fillobject.questions.slice((this.currentPage - 1) * this.itemsPerPage, this.currentPage * this.itemsPerPage)" :key="questionIndex">
                            <p class="font questionName">{{ question.title }}</p>
                            <textarea v-model="selectedAnswers[this.currentPage][questionIndex].answers" name="" id="" cols="30" rows="10" style="resize: none;height: 150px; margin-bottom: 10px;" v-if="question.options === ''"></textarea>
                            <div class="selection" v-for="(option, optionIndex) in question.options" :key="optionIndex">
                                <!-- <p>{{ question }}</p> -->
                                <input type="radio" v-model="selectedAnswers[this.currentPage][questionIndex].answers" :value="optionIndex" :name="'A' + questionIndex + optionIndex" value="" style="margin-right: 5px;" v-if="question.type === '單選題'">
                                <input type="checkbox" v-model="selectedAnswers[this.currentPage][questionIndex].answers" :value="optionIndex" name="" id="" style="margin-right: 5px;" v-if="question.type === '多選題'">
